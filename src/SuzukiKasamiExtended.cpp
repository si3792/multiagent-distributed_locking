...
SuzukiKasamiExtended::SuzukiKasamiExtended(const fipa::Agent& self, const std::vector< std::string >& resources) 
...
void SuzukiKasamiExtended::forwardToken(const std::string& resource)
{
    ...
    if(mOwnedResources[resource] != mSelf.identifier)
    {
        ...
        sendToken(fipa::acl::AgentID (mOwnedResources[resource]), resource, mSelf.identifier + "_" + boost::lexical_cast<std::string>(mConversationIDnum++));
    }
    else
    {
...
bool SuzukiKasamiExtended::isTokenHolder(const std::string& resource, const std::string& agentName)
{
    return mTokenHolders[resource] == agentName;
}
...
void SuzukiKasamiExtended::handleIncomingResponse(const acl::ACLMessage& message)
{
    ...
    if(mOwnedResources[resource] == mSelf.identifier)
    {
        ...
        mTokenHolders[resource] = mSelf.identifier;
    }
...
void SuzukiKasamiExtended::lock(const std::string& resource, const std::list< Agent >& agents)
{
